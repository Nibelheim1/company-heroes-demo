...
function renderResult() {
  ...
  app.innerHTML = shell(`<main class="result-screen"><header class="result-hero"><button class="back-float" data-action="go" data-screen="home">←</button><div class="result-art">${portrait(h,'result-portrait',false)}<div class="result-glow"></div></div><div class="result-copy"><span class="eyebrow">YOUR OPERATING PERSONA</span><h1>${escapeHtml(r.title)}</h1><p class="subtitle">${escapeHtml(r.subtitle)}</p><p>${escapeHtml(r.copy)}</p><div class="tag-row">${r.tags.map(tag=>`<span>${escapeHtml(tag)}</span>`).join('')}</div></div></header>
    <section class="result-body"><div class="match-card"><span class="eyebrow dark">YOUR COMPANY HERO</span><div class="match-head"><div><h2>${escapeHtml(h.name)}</h2><p>${escapeHtml(h.resultCopy || h.mission)}</p></div><strong>${h.matchScore}<small>/100</small></strong></div><div class="match-reasons">${(h.matchReasons||[]).map(x=>`<span>✓ ${escapeHtml(x)}</span>`).join('')}</div><button class="btn dark" data-action="detail" data-code="${h.code}">查看英雄完整档案</button></div>
      <div class="profile-card"><div class="section-heading"><div><span class="eyebrow dark">8-DIMENSION PROFILE</span><h2>你的能力雷达</h2></div><b>${r.confidence}% 置信度</b></div>${dimensionBars(r)}</div>
      ...
      <div class="action-grid"><button class="btn primary" data-action="go" data-screen="team">用英雄组建战队</button><button class="btn soft" data-action="share-result">分享我的人格</button><button class="text-link" data-action="start-quiz">重新测试</button></div>
...
function renderLobby() {
  const own = state.team.map(heroByCode).filter(Boolean), rival = rivalTeam(), visible = state.friendView?rival:own
  app.innerHTML = shell(`<main class="lobby-screen"><header class="lobby-head"><button class="back-float" data-action="go" data-screen="team">←</button><span class="eyebrow">THE MARKET ARENA · BLIND PICK</span><h1>英雄已经就位</h1><p>开局之前，双方只能看到自己的阵容。</p></header><section class="lobby-board"><div class="team-lane blue"><div class="lane-title"><span>${state.friendView?'好友战队':'我的战队'}</span><b>READY · ${visible.length}/${visible.length}</b></div><div class="lobby-slots">${visible.map(h=>lobbySlot(h)).join('')}</div></div><div class="versus-core"><i></i><b>VS</b><span>BLIND PICK</span></div><div class="team-lane red"><div class="lane-title"><span>${state.friendView?'我的未知阵容':'好友未知阵容'}</span><b>HIDDEN · ${own.length}/${own.length}</b></div><div class="lobby-slots">${own.map(()=>lobbySlot(null,true)).join('')}</div></div><div class="lobby-note"><b>${state.friendView?'好友看到的画面':'你看到的画面'}</b><p>${state.friendView?'好友只看到自己的英雄，你的全部位置显示问号。':'好友的全部英雄保持隐藏，直到PK开始。'}</p></div><button class="btn glass" data-action="friend-view">${state.friendView?'返回我的视角':'预览好友视角'}</button><button class="btn primary" data-action="start-battle">模拟好友准备完毕 · 开始PK</button></section></main>`)
}
