...
const teamShareText = () => { const profile = buildTeamProfile(state.team, state.heroes); const code = inviteCodeForTeam(); const primary = state.result?.primary?.name || state.team.map(heroByCode).filter(Boolean)[0]?.name || '我的英雄'; return `我的同频英雄是${primary}，我的战队画像是「${profile.archetype}」。来测出你的英雄，和我组队 PK：${inviteUrl(code)}（组队码 ${code}）` }
const sameTypeReference = () => { const seed = shortHash(state.answers.join('') || state.result?.primary?.code || 'hero'); return 8 + (parseInt(seed, 36) % 24) }
const matchedTeamCodes = () => [...new Set((state.result?.top5 || []).map(hero => hero.code).filter(Boolean))].slice(0, 5)
...
function renderHome() {
  ...
  const resume = state.result ? `<button class="resume-card" data-action="go" data-screen="result"><span>继续查看</span><strong>${escapeHtml(state.result.title)}</strong><small>你的固定人格与同频英雄 →</small></button>` : ''
  const inviteBanner = state.inviteCode ? `<div class="invite-banner"><div><span class="eyebrow dark">FRIEND INVITE</span><strong>你的朋友邀请你来测公司英雄</strong><p>测出你的 A 股英雄后，你们可以直接进行人格 PK。</p></div><button class="btn primary" data-action="start-quiz">接住邀请，开始测试</button></div>` : ''
  ...
  app.innerHTML = shell(`<main>
    <header class="home-hero"><div class="noise"></div><div class="topline"><b>同花顺英雄</b><span>COMPANY HEROES · FULL H5</span></div>
      <div class="hero-stage"><div class="hero-copy"><span class="eyebrow">320 A-SHARE HEROES</span><h1>测出你的底层人格，<br>找到你的 A 股英雄</h1><p>16道经营抉择，先找到不轻易改变的你；再匹配最契合的公司英雄，和朋友进行人格 PK。</p><div class="cta-row"><button class="btn primary" data-action="start-quiz">开始人格测试</button><button class="btn glass" data-action="go" data-screen="heroes">浏览群英录</button></div></div>
    ...
    <section class="home-body">${inviteBanner}${resume}<div class="metrics"><div><strong>320</strong><span>位公司英雄</span></div><div><strong>16×3</strong><span>人格抉择</span></div><div><strong>8</strong><span>人格维度</span></div><div><strong>5</strong><span>人固定战队</span></div></div>
      <div class="heat-board"><div class="section-heading"><div><span class="eyebrow dark">SOCIAL HEAT</span><h2>今天大家匹配到谁？</h2></div><span class="heat-live">LIVE</span></div><p>先测出你的经营人格，再看看你和朋友会不会匹配到同一位英雄。</p><div class="heat-list">${heatHeroes}</div></div>
...
function renderResult() {
  ...
  app.innerHTML = shell(`<main class="result-screen"><header class="result-hero"><button class="back-float" data-action="go" data-screen="home">←</button><div class="result-art">${portrait(h,'result-portrait',false)}<div class="result-glow"></div></div><div class="result-copy"><span class="eyebrow">YOUR OPERATING PERSONA</span><h1>${escapeHtml(r.typeCode || 'HERO')}</h1><p class="persona-name">${escapeHtml(r.title)}</p><p class="subtitle">${escapeHtml(r.subtitle)}</p><div class="type-code-block"><span>HERO TYPE · 你的四字人格</span><small>${(r.typeAxes || []).map(escapeHtml).join(' · ')}</small></div><p>${escapeHtml(r.copy)}</p><div class="tag-row">${r.tags.map(tag=>`<span>${escapeHtml(tag)}</span>`).join('')}</div></div></header>
    <section class="result-body"><div class="persona-proof"><div><span class="eyebrow dark">YOUR HERO IDENTITY</span><h2>你的同频英雄是 ${escapeHtml(h.name)}</h2><p>这是与你最同频的 A 股公司人格。你可以继续补充同频英雄，但不会改变你的底层人格。</p></div><div class="same-type"><strong>约 ${sameTypeReference()}%</strong><span>玩法样本同型参考</span></div></div>
      <div class="profile-card"><div class="section-heading"><div><span class="eyebrow dark">YOUR FIXED PERSONA</span><h2>这是测出来的你</h2></div><b>${r.confidence}% 置信度</b></div><p class="profile-intro">人格是你的底层倾向，不会因为换一家公司就被改写；英雄只是让朋友一眼看懂你的方式。</p>${dimensionBars(r)}</div>
      <div class="team-cta"><div class="team-cta-art">${portrait(h,'team-cta-image',true)}</div><div><span class="eyebrow dark">NEXT MOVE</span><h2>画像完成，组建你的英雄战队</h2><p>先把最同频的英雄带入战队，再决定要不要加入更多人格切面。</p></div><button class="btn primary" data-action="prepare-team">用同频英雄组队</button></div>
      ...
      <div class="share-card"><div><span class="eyebrow dark">YOUR SHARE CARD</span><h2>把“我是谁”发给朋友</h2><p>朋友测出自己的经营人格后，你们可以比较谁更像哪位英雄。</p></div><div class="share-code"><span>好友邀请码</span><strong>${inviteCodeForResult()}</strong></div><div class="share-actions"><button class="btn primary" data-action="share-result">分享我的结果</button><button class="btn soft" data-action="copy-result-invite">复制邀请文案</button></div></div>
...
function renderTeam() {
  ...
  app.innerHTML = shell(`<main><header class="team-head"><span class="eyebrow">TEAM BUILDER · 5 HEROES</span><h1>组建你的公司英雄队</h1><p>你的经营人格已经测出；固定选择 5 位英雄，组成你的战队与人格组合。</p></header><section class="team-body"><div class="team-status"><div><span>YOUR ROSTER</span><strong>${team.length}<small>/5</small></strong></div><p>${team.length<5?'还需要选择 '+(5-team.length)+' 位英雄才能进入人格 PK':'五人阵容已满，可以锁定'}</p></div><div class="selected-team">${team.length?team.map(teamMember).join(''):`<div class="empty-roster">从下面选择 5 位英雄<br><small>它们不会改变你的底层人格，只会让战队更有层次</small></div>`}</div><button class="btn primary lock-team" data-action="lock-team" ${team.length<5?'disabled':''}>锁定五人阵容 · 邀请好友 PK</button><div class="team-invite-card"><div><span class="eyebrow dark">TEAM INVITE</span><h2>先发给朋友，再一起测人格</h2><p>组队码 ${inviteCodeForTeam()} · 朋友完成测试后，选择自己的五人战队加入对战。</p></div><button class="btn soft" data-action="copy-team-invite">复制组队邀请</button></div><div class="section-heading"><div><span class="eyebrow dark">SELECT HEROES</span><h2>选择队员</h2></div></div>${renderFilters()}<div class="team-pick-grid">${list.map(h=>`<button class="team-pick ${state.team.includes(h.code)?'selected':''}" data-action="toggle-team" data-code="${h.code}">${portrait(h,'team-pick-image')}<span><b>${escapeHtml(h.name)}</b><small>${escapeHtml(h.archetype)}</small></span><i>${state.team.includes(h.code)?'✓':'+'}</i></button>`).join('')}</div></section></main>`)
}
...
function renderLobby() {
  ...
  app.innerHTML = shell(`<main class="lobby-screen"><header class="lobby-head"><button class="back-float" data-action="go" data-screen="team">←</button><span class="eyebrow">THE MARKET ARENA · BLIND PICK</span><h1>英雄已经就位</h1><p>不是公司对公司，而是你和朋友各自组建的五人英雄战队对战。</p></header><section class="lobby-board"><div class="team-lane blue"><div class="lane-title"><span>我的五人战队</span><b>READY · ${own.length}/5</b></div><div class="lobby-slots">${own.map(h=>lobbySlot(h)).join('')}</div></div><div class="versus-core"><i></i><b>VS</b><span>PERSONA PK</span></div><div class="team-lane red"><div class="lane-title"><span>好友未知阵容</span><b>HIDDEN · 5/5</b></div><div class="lobby-slots">${own.map(()=>lobbySlot(null,true)).join('')}</div></div><div class="lobby-note"><b>人格盲选对战</b><p>你的五人阵容已经锁定，邀请好友也测出人格并选择五位英雄后开始 PK。</p></div><div class="invite-strip"><span>组队码 <strong>${inviteCodeForTeam()}</strong></span><button class="btn glass" data-action="copy-team-invite">复制邀请</button></div><button class="btn primary" data-action="start-battle">邀请好友组队 PK</button></section></main>`)
}
...
function renderBattle() {
  ...
  app.innerHTML = shell(`<main class="battle-screen"><header class="battle-head"><button class="back-float" data-action="go" data-screen="lobby">←</button><span class="eyebrow">5-DAY PERSONA DUEL · DEMO</span><h1>${d.own>=d.rival?`D${d.day}，你的战队暂时领先`:`D${d.day}，好友战队暂时领先`}</h1><p>比较的是你和朋友各自的英雄人格战队，不是公司的投资表现。</p></header><section class="battle-body"><div class="score-board"><div><span>我的战队分</span><strong class="${d.own>=d.rival?'win':''}">${d.own>0?'+':''}${d.own}</strong></div><b>VS</b><div><span>好友战队分</span><strong class="${d.rival>d.own?'win':''}">${d.rival>0?'+':''}${d.rival}</strong></div></div><div class="day-tabs">${b.days.map((x,i)=>`<button class="${i===state.battleDay?'active':''}" data-action="battle-day" data-index="${i}"><span>D${i+1}</span><b>${x.own>0?'+':''}${x.own}</b></button>`).join('')}</div><div class="team-profile-card"><span class="eyebrow">YOUR TEAM PERSONA</span><h2>${escapeHtml(b.profile.archetype)}</h2><p>${escapeHtml(b.profile.copy)} 这就是你带进 PK 的人格底色。</p><div class="chemistry"><b>战队化学反应 · ${b.profile.chemistryScore}</b><p>${escapeHtml(b.profile.chemistry)}</p></div><div class="profile-strengths">${b.profile.strengths.map(x=>`<span>${escapeHtml(x)}</span>`).join('')}</div><div class="blind-spot"><b>战队盲点</b><p>${escapeHtml(b.profile.blindSpot)}</p></div><div class="team-face-row">${b.own.map(h=>portrait(h,'team-face')).join('')}</div></div><div class="action-grid"><button class="btn primary" data-action="share-team">分享我的战队画像</button><button class="btn soft" data-action="copy-team-invite">邀请好友再来一局</button><button class="btn soft" data-action="go" data-screen="team">调整战队阵容</button></div><div class="safety-note"><b>PK规则说明</b><p>正式版可接入合规授权行情作为背景事件，但胜负只服务于人格社交玩法，不代表投资表现。</p></div></section></main>`)
}
...
function handleAction(target) {
  ...
  if (action==='prepare-team') { state.team=matchedTeamCodes(); return go('team') }
  if (action==='lock-team') { if(state.team.length<5)return toast(`还需要选择${5-state.team.length}位英雄`); state.friendView=false; return go('lobby') }
