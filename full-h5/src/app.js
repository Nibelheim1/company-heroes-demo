...
function renderResult() {
  ...
  app.innerHTML = shell(`<main class="result-screen"><header class="result-hero"><button class="back-float" data-action="go" data-screen="home">←</button><div class="result-art">${portrait(h,'result-portrait',false)}<div class="result-glow"></div></div><div class="result-copy"><span class="eyebrow">YOUR OPERATING PERSONA</span><h1>${escapeHtml(r.title)}</h1><p class="subtitle">${escapeHtml(r.subtitle)}</p><div class="type-code-block"><span>HERO TYPE · 你的四字人格</span><strong>${escapeHtml(r.typeCode || 'HERO')}</strong><small>${(r.typeAxes || []).map(escapeHtml).join(' · ')}</small></div><p>${escapeHtml(r.copy)}</p><div class="tag-row">${r.tags.map(tag=>`<span>${escapeHtml(tag)}</span>`).join('')}</div></div></header>
    <section class="result-body"><div class="profile-card"><div class="section-heading"><div><span class="eyebrow dark">YOUR FIXED PERSONA</span><h2>这是测出来的你</h2></div><b>${r.confidence}% 置信度</b></div><p class="profile-intro">人格是你的底层倾向，不会因为换一家公司就被改写；英雄只是让朋友一眼看懂你的方式。</p>${dimensionBars(r)}</div>
      <div class="persona-proof"><div><span class="eyebrow dark">YOUR HERO IDENTITY</span><h2>你的同频英雄是 ${escapeHtml(h.name)}</h2><p>这是与你最同频的 A 股公司人格。你可以继续补充同频英雄，但不会改变你的底层人格。</p></div><div class="same-type"><strong>约 ${sameTypeReference()}%</strong><span>玩法样本同型参考</span></div></div>
